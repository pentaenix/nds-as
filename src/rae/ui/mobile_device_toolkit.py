...
def _mobile_fetch_finished(window, target: Path, summary: dict) -> None:
    ...
    open_now = QMessageBox.question(
        window,
        "Mobile ROM created",
        f"Created:\n{target}\n\nOpen this ROM in RAE now?",
    )
    if open_now == QMessageBox.Yes:
        if hasattr(window, "open_rom_path"):
            window.open_rom_path(str(target))
        else:
            QMessageBox.information(window, "Open manually", f"Use File → Open ROM and choose:\n{target}")
...
def fetch_android_app_rom(adb: str, serial: str, package_id: str, target: Path, *, progress: Callable[[str], None]) -> dict:
    ...
    (target / "rae_mobile_rom.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False), encoding="utf-8")
    (target / "probe" / "apk_paths.txt").write_text("\n".join(package_paths) + "\n", encoding="utf-8")

    for remote in package_paths:
        local = target / "apk" / Path(remote).name
    ...
    _probe_listing(adb_base, f"/sdcard/Android/data/{package_id}", target / "probe" / "android_data_listing.txt", progress)
    _probe_listing(adb_base, f"/sdcard/Android/obb/{package_id}", target / "probe" / "obb_listing.txt", progress)

    optional_dirs = [
        (f"/sdcard/Android/data/{package_id}/files", target / "external_files", "external files"),
        (f"/sdcard/Android/data/{package_id}/cache", target / "external_cache", "external cache"),
        (f"/sdcard/Android/obb/{package_id}", target / "obb", "OBB"),
    ]
    ...
    (target / "probe" / "candidate_files.txt").write_text("\n".join(candidates) + ("\n" if candidates else ""), encoding="utf-8")
    ...
    _write_readme(target, package_id, app_name)
    _try_build_inventory(target, package_id, progress, summary)
    (target / "probe" / "fetch_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    progress(f"Fetch summary: {target / 'probe' / 'fetch_summary.json'}")
    return summary
...
def _write_readme(target: Path, package_id: str, app_name: str) -> None:
    (target / "README.md").write_text(
        f"# {app_name}.rom\n\n"
        f"This is a RAE mobile app ROM bundle for `{package_id}`.\n\n"
        "It is intentionally a directory with a `.rom` extension. RAE opens it through File → Open ROM, just like console ROM sources.\n\n"
...
def install_mobile_device_toolkit(window) -> None:
    """Install the generic Device Toolkit menu and a toolbar fallback.
    ...
    # Toolbar fallback: if macOS native menu placement is confusing or hidden under
    # the Python app menu, the action is still visible in RAE's main toolbar.
    QTimer.singleShot(0, lambda: _install_device_toolkit_toolbar_fallback(window))


def _install_device_toolkit_toolbar_fallback(window) -> None:
    from PySide6.QtGui import QAction
    ...
    for action in toolbar.actions():
        if action.text().replace("&", "") == "Extract App ROM":
            return
    action = QAction("Extract App ROM", window)
    action.setToolTip("Extract an installed Android app into a generic .rom bundle under roms/.")
    action.triggered.connect(lambda: _extract_installed_app_rom(window))
