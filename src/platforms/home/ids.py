...
CAP_ID_RE = re.compile(r"cap(?P<num>\d{4})_f(?P<form_a>\d{2})", re.IGNORECASE)
MITAKE_PREVIEW_RE = re.compile(r"mt_pv_ev_(?P<num>\d{4})_(?P<form_a>\d{2})_(?P<form_b>\d{2})", re.IGNORECASE)
...
def parse_home_cap_id(text: str) -> HomePokemonId | None:
    match = CAP_ID_RE.search(text or "")
    if not match:
        return None
    return HomePokemonId(
        raw=match.group(0),
        number=int(match.group("num")),
        form_a=(match.group("form_a") or "00").zfill(2),
        form_b="00",
    )


def parse_home_mitake_preview_id(text: str) -> HomePokemonId | None:
    match = MITAKE_PREVIEW_RE.search(text or "")
    if not match:
        return None
    return HomePokemonId(
        raw=match.group(0),
        number=int(match.group("num")),
        form_a=(match.group("form_a") or "00").zfill(2),
        form_b=(match.group("form_b") or "00").zfill(2),
    )


def parse_home_asset_id(text: str) -> HomePokemonId | None:
    for parser in (parse_home_pokemon_id, parse_home_cap_id, parse_home_mitake_preview_id):
        parsed = parser(text)
