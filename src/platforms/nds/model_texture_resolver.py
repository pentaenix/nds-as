...
def _model_context_tokens(manifest: NsbmdManifest, model: Asset) -> set[str]:
    """Tokens that identify this model variant (e.g. batt_bg04d → bg04, field04)."""
    tokens: set[str] = set()
    sources: list[str] = list(manifest.model_names)
    for material in manifest.materials:
        if material.texture_name:
            sources.append(material.texture_name)
        if material.material_name:
            sources.append(material.material_name)
    for name in sources:
        folded = name.casefold()
        if len(folded) >= 3:
            tokens.add(folded)
        for match in re.finditer(r"(?:bg|field|stage)(\d+)", folded):
            digits = match.group(1)
            tokens.add(digits)
            tokens.add(f"bg{digits}")
            tokens.add(f"field{digits}")
    stem = Path(model.virtual_path.split("#", 1)[0]).stem.casefold()
    if stem and not stem.startswith("file_"):
        tokens.add(stem)
    return {token for token in tokens if len(token) >= 2}


def _texture_asset_name_rank(
    model: Asset,
    texture_asset: Asset | None,
    manifest: NsbmdManifest | None,
) -> int:
    if manifest is None or texture_asset is None:
        return 1
    tokens = _model_context_tokens(manifest, model)
    if not tokens:
        return 1
    haystacks = [
        texture_asset.virtual_path.casefold(),
        (texture_asset.mapping_label or "").casefold(),
    ]
    return 0 if any(token in hay for token in tokens for hay in haystacks) else 1


def _rank_btx_asset_for_model(
    model: Asset,
    texture_asset: Asset,
    *,
    manifest: NsbmdManifest | None = None,
) -> tuple[int, int, int, str]:
    model_path = model.virtual_path
    texture_path = texture_asset.virtual_path
    ...
    number_delta = (
        abs(model_number - texture_number)
        if model_number is not None and texture_number is not None
        else 999_999
    )
    same_number = model_number is not None and model_number == texture_number

    if texture_asset.asset_id == model.asset_id:
        context = 0
    elif model.folder_key and texture_asset.folder_key == model.folder_key:
        context = 1
    elif (
        model.container_chain
        and texture_asset.container_chain
        and texture_asset.container_chain[: len(model.container_chain)] == model.container_chain
    ):
        context = 2
    elif model_archive and texture_archive and model_archive == texture_archive:
        context = 3
    elif same_number and texture_asset.mapping_category == "textures":
        context = 4
    elif same_number:
        context = 5
...
def _sibling_btx0_resolution(
    ...
    *,
    ...
    ranked_siblings = sorted(
        siblings,
        key=lambda btx: _rank_btx_asset_for_model(model, btx, manifest=manifest),
    )
    decoded_images: list[DecodedImage] = []
    resolved_assets: list[Asset] = []
    fulfilled: set[tuple[str, str]] = set()
    request_keys = {(tex.casefold(), (pal or "").casefold()) for tex, pal in requests}

    for btx in ranked_siblings:
        remaining = [
            (tex, pal)
            for tex, pal in requests
            if (tex.casefold(), (pal or "").casefold()) not in fulfilled
        ]
        if not remaining:
            break
        images = decode_guided_tex0_images(
            btx.data,
            texture_requests=remaining,
            max_images=policy.max_decoded_images or 128,
        )
        if not images:
            continue
        ...
        for img in images:
            key = (img.name.casefold(), (img.palette_name or "").casefold())
            if key in fulfilled:
                continue
            fulfilled.add(key)
            decoded_images.append(img)
            if policy.max_decoded_images is not None and len(decoded_images) >= policy.max_decoded_images:
                break
        if policy.max_decoded_images is not None and len(decoded_images) >= policy.max_decoded_images:
            break
    ...
    if not decoded_images:
        return None
    if fulfilled != request_keys:
        lines.append(
            f"- sibling NSBTX decode fulfilled {len(fulfilled)}/{len(request_keys)} requested texture name(s)"
        )
    lines.append(
        f"- sibling NSBTX in the same archive/folder decoded {len(decoded_images)} texture image(s) "
        f"from {len(resolved_assets)} ranked archive(s)"
    )
    bindings = [
        ResolvedMaterialTexture(
            ...
            img.name,
            img.palette_name,
            resolved_assets[0].asset_id,
            resolved_assets[0].virtual_path,
            img,
            "sibling NSBTX in same archive/folder",
        )
