...
from ..gltf.texture_patch import write_flipbook_preview_glbs, write_patched_preview_glb
...
def build_web_preview_glb(
    ...
    *,
    ...
    material_to_texture: dict[str, str] | None = None,
    stage_texture_paths: list[Path] | None = None,
    patcher: PreviewGlbPatcher | None = None,
) -> Path | None:
    if not mesh_labels or not any(mesh_texture_paths):
        return None
    active = patcher or PreviewGlbPatcher()
    return active.build_preview_glb(
        source_glb,
        mesh_labels=mesh_labels,
        mesh_texture_paths=mesh_texture_paths,
        texture_by_name=texture_by_name,
