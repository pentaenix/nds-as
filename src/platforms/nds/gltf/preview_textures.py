"""Deterministic GLB preview texture assignment (format-neutral glTF helpers).

apicula and AssetStudio write GLBs with external PNG URIs beside the model.
RAE reads the glTF material table and attaches colocated PNGs for web/GL preview.
"""
...
_GENERIC_MESH_LABEL = re.compile(r"^mesh_\d+$", re.IGNORECASE)
...
def parse_glb_material_texture_map(glb_path: Path) -> dict[str, Path]:
    ...
    for mat_idx, material in enumerate(materials):
        ...
        path: Path | None = None
        if uri:
            candidate = directory / Path(uri).name
            if candidate.is_file():
                path = candidate
        if path is None:
            for key in _material_embedded_texture_keys(material, textures, images):
                path = _best_path_for_key(
                    key,
                    colocated=colocated,
                    texture_by_name={},
                    glb_path=glb_path,
        ...
        if uri:
            out[Path(uri).stem.casefold()] = path
    return out
...
def prefer_resolver_texture_map(
    resolver_map: dict[str, Path],
    apicula_map: dict[str, Path],
) -> dict[str, Path]:
    """Merge apicula PNG lookups, then let exact resolver bindings override them.

    apicula may decode duplicate texture dictionary names from unrelated sibling
    archives (common in ``a/0/1/1`` battle-background folders). Resolver PNGs are
    ranked for the selected model and must win on name collisions.
    """
    out = merge_texture_by_name(apicula_map)
    for key, path in resolver_map.items():
        out[key.casefold()] = path
    return out
...
def apply_material_preview_alpha(colors, state: MaterialPreviewState | None):
    ...
    if state.alpha_mode == "MASK":
        # MASK cutout is enforced in the texture sampler / web viewer, not vertex bakes.
        out[:, 3] = 0.0
        return out
    return out
...
def _is_generic_mesh_label(label: str) -> bool:
    return bool(_GENERIC_MESH_LABEL.match(str(label or "").strip()))


def _texture_keys_from_image_name(name: str) -> list[str]:
    """Turn apicula image names (e.g. ``BodyA|pm0001_00_BodyA1.tga``) into lookup keys."""
    ...
    seen: set[str] = set()
    for part in parts:
        stem = Path(part).stem.casefold().removesuffix(".tga")
        for candidate in (stem, texture_key_for_path(Path(stem))):
...
def _material_embedded_texture_keys(material: dict, textures: list, images: list) -> list[str]:
    pbr = material.get("pbrMetallicRoughness") or {}
    tex_info = pbr.get("baseColorTexture") or {}
    tex_idx = tex_info.get("index")
    if tex_idx is None:
    ...
    try:
        ...
        if image_idx is None:
            return []
        image = images[int(image_idx)]
    except (IndexError, TypeError, ValueError):
        return []
    if not isinstance(image, dict):
        return []
    return _texture_keys_from_image_name(str(image.get("name") or ""))
...
def _path_for_glb_part(
    ...
    *,
    ...
    # 1b. Embedded glTF images (bufferView) — map via apicula image names to resolver PNGs.
    if glb_path is not None and glb_path.is_file():
        try:
            ...
            if material_index is not None and 0 <= material_index < len(materials):
                material = materials[material_index]
                if isinstance(material, dict):
                    embedded_keys.extend(_material_embedded_texture_keys(material, textures, images))
            if mat_key:
                embedded_keys.append(mat_key)
            seen_embedded: set[str] = set()
            for key in embedded_keys:
                lookup = key.casefold()
                if not lookup or lookup in seen_embedded:
                    continue
                seen_embedded.add(lookup)
                path = _best_path_for_key(
                    lookup,
                    colocated=colocated,
                    texture_by_name=texture_by_name,
    ...
    if _is_generic_mesh_label(mat_key) and texture_bind_order:
    ...
    if _is_generic_mesh_label(mat_key) and fallback_paths:
