...
from ...core.platform_features import supports_easyfind
...
class EasyFindActionsMixin:
    ...
    def _update_main_toolbar(self) -> None:
        ...
        if in_easyfind:
            ...
            self.easyfind_action.setVisible(
                loaded and self._current_game_code() != "" and easyfind_enabled
            )
            self.easyfind_back_action.setVisible(False)
            self.easyfind_build_action.setVisible(False)
            self.easyfind_fit_action.setVisible(False)
    ...
    def open_easyfind_workspace(self) -> None:
        if not supports_easyfind(getattr(self, "_rom_platform_id", None)):
            QMessageBox.information(
                self,
                "EasyFind unavailable",
                "EasyFind is only available for Nintendo DS ROMs.",
            )
            return
