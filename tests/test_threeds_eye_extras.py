...
def test_material_role_mapping():
    ...
    assert _material_role("Mouth") == "mouth"
...
def test_attach_material_extras_mouth_sheet():
    entry: dict = {"name": "Mouth"}
    mat = GfMaterial(name="Mouth", texture_names=[], texture_units=[])
    _attach_material_extras(
        entry,
        mat,
        "opaque",
    ...
    rae = entry["extras"]["rae"]
    assert rae["materialRole"] == "mouth"
    assert rae["eyeSheet"]["uvLayout"] == "raw_u"
    assert rae["eyeExpression"]["frameCount"] == 8


def test_mesh_uv_bake_matches_gf_maya_texture_matrix():
    assert _bake_mesh_uv(0.25, 0.75, 2.0, 4.0, 0.0, 1.0, 0.5) == pytest.approx(
        (-1.5, 0.0)
    )
    assert _bake_mesh_uv(1.1, 1.9, -2.0, 4.0, 0.0, 0.93, 0.0) == pytest.approx(
        (-0.34, -6.6)
    )
    assert _bake_mesh_uv(
        0.2,
        0.4,
        2.0,
        3.0,
